...
const DashboardUsuario = ({ alRegresar }) => {
  // Datos simulados actualizados al año en curso (2026)
  const [finanzas] = useState({
    totalPrestamo: 5000,
    totalAbonado: 3200,
    cuotasPagadas: 8,
    totalCuotas: 12,
    ultimoAbono: "15 de Mayo, 2026",
    proximoVencimiento: "15 de Junio, 2026",
    historial: [
      { id: 1, fecha: "15/05/26", monto: 400, metodo: "Transferencia" },
      { id: 2, fecha: "15/04/26", monto: 400, metodo: "Efectivo" },
      { id: 3, fecha: "15/03/26", monto: 400, metodo: "Transferencia" },
      { id: 4, fecha: "15/02/26", monto: 400, metodo: "Transferencia" },
    ]
  });

  const pendiente = finanzas.totalPrestamo - finanzas.totalAbonado;
  const porcentajePagado = (finanzas.totalAbonado / finanzas.totalPrestamo) * 100;
            ...
            <div className="bg-slate-800 p-8 text-white">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div>
                  ...
                  <p className="text-3xl font-black">${finanzas.totalPrestamo.toLocaleString()}</p>
                </div>
                <div>
                  <p className="text-emerald-400 text-xs font-bold uppercase">Total Abonado</p>
                  <p className="text-3xl font-black text-emerald-400">${finanzas.totalAbonado.toLocaleString()}</p>
                </div>
                <div>
            ...
            <div className="p-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="space-y-4">
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                  ...
                  <p className="text-xl font-bold text-slate-800">{finanzas.cuotasPagadas} / {finanzas.totalCuotas}</p>
                </div>
                <div className="bg-emerald-50 p-4 rounded-2xl border border-emerald-100">
                  <p className="text-emerald-700 text-[10px] font-bold uppercase">Próximo Pago</p>
                  <p className="text-xl font-bold text-slate-800">{finanzas.proximoVencimiento}</p>
                </div>
              </div>
